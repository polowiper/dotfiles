...
class dOffset(idaapi.plugin_t):
    ...
    def AttachActionToMenu(self):
        idaapi.attach_action_to_menu(
            "Jump/dOffset/", self.ACTION_NAME_JUMPTOOFFSET, idaapi.SETMENU_APP
        )
...
class Hooks(idaapi.UI_Hooks):
    ...
    def hxe_callback(self, event, *args):
        if event == idaapi.hxe_populating_popup:
            ...
            idaapi.attach_action_to_popup(
                form, popup, dOffset.ACTION_NAME_MAINCTX, "dOffset", idaapi.SETMENU_APP
            )
            idaapi.attach_action_to_popup(
                form,
                popup,
...
class GetModuleOffsetHandler(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        print(
            "[%s] %X -> %s+0x%X"
            % (PLUGIN_NAME, idaapi.get_screen_ea(), file_name, CurrentOffset)
        )
        return 1
...
class JumpToOffsetHandler(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if idaapi.jumpto(AddressToJump):
            ...
            print(
                "[%s] Failed to jump to address with offset 0x%X"
                % (PLUGIN_NAME, InputOffsetInt)
            )
